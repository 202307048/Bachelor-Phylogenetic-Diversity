...
#Table 1: Cell distribution ----
#Create contingency table from cell distributions
contingency_table <- matrix(
  c(2, 689, 1126, 697,      # Terrestrial
    0, 19925, 1731, 163),   # Marine
  nrow = 2, 
  byrow = TRUE,  #fills matrix row by row
  dimnames = list(
    Realm = c("Terrestrial", "Marine"),
    Category = c("Strongly Redundant", "Slightly Redundant", 
                 "Slightly Unique", "Strongly Unique")
  )
)
...
residual_contributions <- chi_test$residuals^2
print(round(residual_contributions, 2))
...
F2_A <- ggplot(results_terr, aes(x = SES.Mean, y = SES.SD)) +
  ...
  geom_vline(xintercept = 2, linetype = "dashed", color = "black") + #Black line at 2
  geom_vline(xintercept = -2, linetype = "dashed", color = "black") + #Black line at -2 
  labs(
    title = "(a)",
    x = "SES mean",
